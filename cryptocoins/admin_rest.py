...
from core.consts.currencies import BEP20_CURRENCIES, ERC20_MATIC_CURRENCIES, ERC20_ULC_CURRENCIES
...
from cryptocoins.coins.ulc import ULC_CURRENCY
...
from cryptocoins.models.proxy import BNBWithdrawalApprove, MaticWithdrawalApprove, UlcWithdrawalApprove
...
from cryptocoins.serializers import UlcKeySerializer
...
@api_admin.register(UlcWithdrawalApprove)
class UlcWithdrawalApproveApiAdmin(BaseWithdrawalApprove):
    def get_queryset(self):
        return get_withdrawal_requests_to_process(
            [ULC_CURRENCY, *ERC20_ULC_CURRENCIES],
            blockchain_currency='ULC'
        )

    @api_admin.action(permissions=True)
    def process(self, request, queryset):
        serializer = UlcKeySerializer(data=request.data)
        if serializer.is_valid(raise_exception=True):
            password = request.data.get('key')
            process_payouts_task.apply_async(['ULC', password, ], queue='ulc_payouts')
